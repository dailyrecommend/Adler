...
namespace Adler.Aircraft
{
    ...
    public sealed class AirframeDefinition : ScriptableObject
    {
        ...
        [Tooltip("부스터를 밟은 뒤 그 속도에 다다르기까지 걸리는 시간(초).\n\n" +
                 "이 동안에는 평소 가속을 쓰지 않는다. 부스터는 유일한 가속 수단이라\n" +
                 "눌렀는데 한참 뒤에 빨라지면 통했는지를 소리로만 짐작하게 되고,\n" +
                 "그렇다고 순간이동시키면 밟은 것이 사건이 아니라 상태 전환이 된다.\n\n" +
                 "카메라가 튕겼다 돌아오는 시간과 맞춰두면 둘이 한 사건으로 읽힌다.\n" +
                 "0으로 두면 예전처럼 즉발이다.")]
        [Min(0f)]
        public float BoostSurgeSeconds = 0.2f;
